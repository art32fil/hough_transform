...
inline shared_ptr<_set_> find_maxs(const vector<T>& array) {
  ...
  const int max_iter = array.size()/16;
  for (long  i = -(array.size()/16); i < (long) array.size()/16; i++) {
    int j = 0;
...
int generate_map(const RobotState& init_pose, const TransformedLaserScan& scan,
  ...
  for (auto& sp : scan.points) {
    if (ii%3 == 0) {
      ...
      DiscretePoint2D dsp = map.world_to_cell(x,y);
...
double HoughScanMatcher::process_scan(const RobotState &init_pose,
  ...
  HoughTransform scan_HT(720, 0.1);
  ...
  for (size_t i = 1; i < scan.points.size(); i++) {
    ...
    double x = scan.points[i].range * cos(scan.points[i].angle);
    double y = scan.points[i].range * sin(scan.points[i].angle);
    scan_HT.transform(map.world_to_cell(x,y));
    i++;
  }
